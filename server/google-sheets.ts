...
class GoogleSheetsService {
    ...
    try {
      ...
      for (let i = 1; i < rows.length; i++) {
        if (rows[i][2]?.toString().trim().toLowerCase() === email.trim().toLowerCase()) {
          existingRow = i + 1; // 1-based 시트 행
          break;
        }
      }

      const record = [[region, memberName, email, password, auth]]; // A B C D E

      if (existingRow > 0) {
        // 기존 행 업데이트 (중복 append 방지) — RAW로 저장해 PW 앞자리 0 보존
      ...
      // 신규 append — RAW로 저장해 PW 앞자리 0 보존
  ...
  async addWithdrawalHistory(userEmail: string, region: string, chapter: string, memberName: string, adminEmail?: string): Promise<void> {
    try {
      ...
      // 1. RPS 시트에서 해당 유저의 전체 행 데이터(A~Z) 읽기
      ...
      if (headerResp.ok) {
        ...
        if (!hd.values || hd.values.length === 0) {
          ...
          if (rpsHeaderResp.ok) {
            const rhd = await rpsHeaderResp.json();
      ...
      console.log(`✅ 탈퇴 히스토리 기록 완료 (전체 데이터): ${userEmail} (${withdrawalTime}), 저장 열 수: ${rowToAppend.length}, fullRowData 열 수: ${fullRowData.length}`);
    ...
    return requestQueue.enqueue(
      ...
      async () => {
        ...
        try {
          ...
          // 2. 삭제할 사용자들의 행 인덱스와 전체 행 데이터 수집
          const headerRow = rows[0] || [];
            ...
            for (let i = 1; i < rows.length; i++) {
              if (rows[i] && rows[i][0] &&
                ...
                usersToDelete.push({
                  ...
                  chapter: rows[i][2] || '',
                  memberName: rows[i][3] || '',
                  fullRow: rows[i].map((cell: any) => cell?.toString() || '')
                });
                found = true;
                break;
              }
  ...
  async deleteAdminFromSheet(email: string): Promise<void> {
    ...
    await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${this.spreadsheetId}:batchUpdate`,
      {
        ...
        headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          requests: [{ deleteDimension: { range: { sheetId: adminSheet.properties.sheetId, dimension: 'ROWS', startIndex: sheetRowNumber - 1, endIndex: sheetRowNumber } } }]
        })
      }
    );
  }
  ...
  async deleteChapterFromMaster(chapter: string): Promise<void> {
    ...
    await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${this.spreadsheetId}:batchUpdate`,
      {
        ...
        headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          requests: [{ deleteDimension: { range: { sheetId: masterSheet.properties.sheetId, dimension: 'ROWS', startIndex: sheetRowNumber - 1, endIndex: sheetRowNumber } } }]
        })
      }
    );
  }

  async fixWithdrawalHistoryHeader(): Promise<void> {
    const accessToken = await this.getAccessToken();
    // RPS 헤더 읽기
    const rpsHeaderResp = await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${this.spreadsheetId}/values/RPS!A1:Z1`,
    ...
    if (!rpsHeaderResp.ok) throw new Error('RPS 헤더를 읽을 수 없습니다');
    const rhd = await rpsHeaderResp.json();
    ...
    console.log(`✅ WithdrawalHistory 헤더 업데이트 완료: ${newHeader.length}열`);
  }
  ...
  async restoreMemberFromHistory(email: string, region: string, chapter: string, memberName: string): Promise<void> {
    ...
    if (!alreadyExists) {
      if (rpsRowData.length >= 10) {
        // 새 형식: 전체 행 데이터를 데이터 영역 바로 다음 행에 PUT
        const rpsDataResp = await fetch(
          `https://sheets.googleapis.com/v4/spreadsheets/${this.spreadsheetId}/values/RPS!A1:A5000`,
          { headers: { 'Authorization': `Bearer ${accessToken}` } }
        );
        let targetRow = 2;
        ...
        if (!putResp.ok) {
          ...
          console.error(`❌ RPS PUT FAILED: ${putResp.status} ${errText}`);
          throw new Error(`RPS 복원 실패: ${putResp.status}`);
        }
        console.log(`✅ Full row restored to RPS row ${targetRow}: ${email} (${rpsRowData.length} columns)`);
      } else {
        // 이전 형식: addNewUser로 기본 데이터만 복원
        try {
          ...
          console.log(`✅ Basic restore to RPS via addNewUser: ${email}`);
        } catch (e: any) {
  ...
  async updateBoardPost(rowIndex: number, content: string): Promise<void> {
    const accessToken = await this.getAccessToken();
    await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${this.spreadsheetId}/values/BoardLog!F${rowIndex}?valueInputOption=RAW`, {
      method: 'PUT', headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
  ...
  async getMasterNotices(): Promise<any[]> {
    try {
      ...
      const checkResp = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${this.spreadsheetId}/values/${encodeURIComponent(sheetName)}!A1?access_token=${accessToken}`);
      if (!checkResp.ok) {
        await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${this.spreadsheetId}:batchUpdate`, { method: 'POST', headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ requests: [{ addSheet: { properties: { title: sheetName } } }] }) });
        await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${this.spreadsheetId}/values/${encodeURIComponent(sheetName)}!A1:B1?valueInputOption=RAW`, { method: 'PUT', headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ values: [['Timestamp', 'Content']] }) });
        return [];
      }
      const resp = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${this.spreadsheetId}/values/${encodeURIComponent(sheetName)}!A2:B100`, { headers: { 'Authorization': `Bearer ${accessToken}` } });
      if (!resp.ok) return [];
      const data = await resp.json();
      return (data.values || []).filter((row: any[]) => row[1]?.toString().trim()).map((row: any[]) => ({ timestamp: row[0]?.toString().trim() || '', content: row[1]?.toString().trim() || '' }));
    } catch (error) { return []; }
  }
  ...
  async getBoardPosts(): Promise<any[]> {
    try {
      ...
      if (!checkResp.ok) {
        ...
        await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${this.spreadsheetId}/values/${encodeURIComponent(sheetName)}!A1:G1?valueInputOption=RAW`, {
          method: 'PUT', headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({ values: [['Timestamp', 'Email', 'Name', 'Role', 'Type', 'Content', 'ParentIndex']] })
        });
        return [];
      }
      const resp = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${this.spreadsheetId}/values/${encodeURIComponent(sheetName)}!A2:G500`, { headers: { 'Authorization': `Bearer ${accessToken}` } });
      if (!resp.ok) return [];
      const data = await resp.json();
      return (data.values || []).map((row: any[], i: number) => ({ index: i + 2, timestamp: row[0] || '', email: row[1] || '', name: row[2] || '', role: row[3] || '', type: row[4] || '', content: row[5] || '', parentIndex: row[6] || '' }));
    } catch (error) { return []; }
  }
  ...
  async addBoardPost(email: string, name: string, role: string, type: string, content: string, parentIndex: string = ''): Promise<void> {
    ...
    await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${this.spreadsheetId}/values/${encodeURIComponent('BoardLog')}!A:G:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS`, {
      method: 'POST', headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ values: [[timestamp, email, name, role, type, content, parentIndex]] })
    });
  }
