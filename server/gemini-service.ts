...
• 서비스 및 컨설팅: 전문 서비스 제공 및 경영 컨설팅`;
  ...
  async searchRegionalBusinesses(searchQuery: string, userSpecialty: string = '일반', userRegion: string = '강남구'): Promise<{ businesses: NaverPlaceBusiness[] }> {
    ...
    try {
      ...
      const synergyFields = this.extractSynergyFieldsFromAnalysis(searchQuery, userSpecialty);
...
실제 업체 5개:`]
   ...
   */
  private extractSynergyFieldsFromAnalysis(analysisText: string, userSpecialty: string): string[] {
