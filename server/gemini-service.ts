...
중요: 업체명이 아닌 업종만 나열하고, 쉼표로만 구분하세요.`;
    ...
    // AI 추출 실패 시 분석 텍스트에서 키워드 추출
    if (analysisText && analysisText.length > 50) {
      const extractedFields: string[] = [];
      const keywords = ['카페', '레스토랑', '마케팅', '유통', '관광', '교육', '물류', '포장', '가공', '디자인', '광고', '컨설팅', '의료', '법무', '건축', 'IT', '금융'];
      
      for (const keyword of keywords) {
        if (analysisText.includes(keyword)) {
          extractedFields.push(keyword + '업체');
          if (extractedFields.length >= 8) break;
        }
      }

      if (extractedFields.length > 0) {
        console.log(`🎯 텍스트 분석 성공 - "${userSpecialty}": [${extractedFields.join(', ')}]`);
        return extractedFields;
      }
    }
