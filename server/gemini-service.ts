...
- 최소 5개 이상 추천`
    ...
    // 실제 데이터만 제공 - 가짜 데이터 절대 제공 안함
    console.log(`❌ 실제 업체 검색 서비스 불안정 - 가짜 데이터 제공 거부`);
    throw new Error(`실제 존재하는 업체만을 검색하여 제공하는 정책에 따라, 현재 외부 검색 서비스가 불안정한 상황에서는 가상의 업체 정보를 제공하지 않습니다. 검증된 실제 데이터만 제공하기 위해 잠시 후 다시 시도해주세요.`);
  }
