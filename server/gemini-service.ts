  ...
  async searchRegionalBusinesses(searchQuery: string, userSpecialty: string = '일반', userRegion: string = '강남구'): Promise<{ businesses: any[] }> {
    try {
      ...
      const response = await this.ai.models.generateContent({
        ...
        contents: [
          `당신은 한국 지역별 비즈니스 검색 전문가입니다. 
          
사용자 정보:
- 전문분야: ${userSpecialty}
- 지역: ${userRegion}

위 정보를 바탕으로 ${userRegion} 지역에서 "${userSpecialty}" 전문가와 시너지를 낼 수 있는 업체 10개를 JSON 형태로 추천해주세요.
  ...
  "businesses": [
    {
      "name": "업체명",
      "category": "업종",
      "address": "서울 ${userRegion} 상세주소",
      "phone": "연락처",
      "website": "https://www.example.com",
      "synergyPotential": "시너지 가능성 설명",
      "description": "업체 설명"
    }
  ]
}

중요: ${userSpecialty} 전문분야와 실제로 협업 가능한 업체들을 추천하고, 각 업체마다 웹사이트 URL을 포함해주세요.`
        ]
      });
      ...
      if (rawJson) {
        try {
          ...
          if (data && data.businesses && Array.isArray(data.businesses)) {
            ...
            return this.generateDynamicBusinessResponse(userSpecialty, userRegion);
          }
        } catch (parseError) {
  ...
  private generateDynamicBusinessResponse(userSpecialty: string = '', userRegion: string = '') {
    // 사용자 정보가 없으면 오류 반환
    if (!userSpecialty || !userRegion) {
      throw new Error('사용자의 전문분야 또는 지역 정보가 확인되지 않습니다. 프로필을 확인해주세요.');
    }

    console.log(`🏗️ 동적 업체 생성 - 전문분야: ${userSpecialty}, 지역: ${userRegion}`);

    // 딸기농장운영 특화 시너지 업체 생성
    if (userSpecialty.includes('딸기') || userSpecialty.includes('농장')) {
      return {
        businesses: [
          {
            name: "강남 프리미엄 베이커리",
            category: "베이커리 & 디저트",
            address: `서울 ${userRegion} 베이커리거리 123-45`,
            phone: "02-1234-5678",
            website: "https://www.gangnam-bakery.co.kr",
            synergyPotential: "신선한 딸기를 활용한 프리미엄 디저트 및 케이크 제작",
            description: "고급 베이커리 및 맞춤 케이크 전문점"
          },
          {
            name: "테헤란로 식자재 유통센터",
            category: "식자재 유통",
            address: `서울 ${userRegion} 테헤란로 234-56`,
            phone: "02-2345-6789",
            website: "https://www.teheran-food.co.kr",
            synergyPotential: "레스토랑 및 카페 대상 프리미엄 딸기 정기 납품",
            description: "B2B 전문 식자재 유통 및 납품업체"
          },
          {
            name: "강남 농산물 가공센터",
            category: "농산물 가공",
            address: `서울 ${userRegion} 가공단지 345-67`,
            phone: "02-3456-7890",
            website: "https://www.gangnam-process.co.kr",
            synergyPotential: "딸기잼, 딸기소스 등 가공식품 OEM 생산",
            description: "친환경 농산물 가공 및 포장 전문업체"
          },
          {
            name: "청담 고급 레스토랑",
            category: "고급 요리",
            address: `서울 ${userRegion} 청담로 456-78`,
            phone: "02-4567-8901",
            website: "https://www.cheongdam-fine.co.kr",
            synergyPotential: "시즌별 특선 메뉴 및 디저트 코스 딸기 공급",
            description: "미쉘린 가이드 등재 고급 레스토랑"
          },
          {
            name: "강남 유기농 화장품연구소",
            category: "천연 화장품",
            address: `서울 ${userRegion} 연구단지 567-89`,
            phone: "02-5678-9012",
            website: "https://www.organic-cosmetics.co.kr",
            synergyPotential: "딸기 추출물을 활용한 천연 화장품 원료 개발",
            description: "유기농 원료 기반 화장품 연구 및 제조"
          },
          {
            name: "강남 체험관광센터",
            category: "관광 & 체험",
            address: `서울 ${userRegion} 관광로 678-90`,
            phone: "02-6789-0123",
            website: "https://www.gangnam-tour.co.kr",
            synergyPotential: "딸기농장 체험투어 패키지 기획 및 운영",
            description: "지역 특화 체험관광 프로그램 전문업체"
          },
          {
            name: "프리미엄 건강식품연구소",
            category: "건강식품",
            address: `서울 ${userRegion} 헬스타운 789-01`,
            phone: "02-7890-1234",
            website: "https://www.premium-health.co.kr",
            synergyPotential: "딸기 성분을 활용한 건강기능식품 개발",
            description: "천연 원료 기반 건강기능식품 연구 개발"
          },
          {
            name: "강남 온라인마켓플레이스",
            category: "전자상거래",
            address: `서울 ${userRegion} IT타워 890-12`,
            phone: "02-8901-2345",
            website: "https://www.gangnam-market.co.kr",
            synergyPotential: "온라인 직판 및 농장직송 판매 채널 구축",
            description: "프리미엄 농산물 전문 온라인 판매 플랫폼"
          },
          {
            name: "대치동 컨퍼런스센터",
            category: "이벤트 기획",
            address: `서울 ${userRegion} 컨퍼런스로 901-23`,
            phone: "02-9012-3456",
            website: "https://www.daechi-conference.co.kr",
            synergyPotential: "농업 세미나 및 팜투테이블 이벤트 기획",
            description: "기업 이벤트 및 컨퍼런스 전문 기획업체"
          },
          {
            name: "강남 포장재 솔루션",
            category: "포장재 제조",
            address: `서울 ${userRegion} 포장단지 012-34`,
            phone: "02-0123-4567",
            website: "https://www.gangnam-package.co.kr",
            synergyPotential: "친환경 딸기 포장재 및 브랜딩 패키지 개발",
            description: "친환경 농산물 전용 포장재 설계 및 제조"
          }
        ]
      };
    }
