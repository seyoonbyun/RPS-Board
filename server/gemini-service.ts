...
실제 존재하는 업체만 추천하세요.`
        ...
        throw new Error('Gemini API 서비스에 일시적인 문제가 발생했습니다. 구글 서버 상태를 확인 중이니 잠시 후 다시 시도해주세요.');
      }
  ...
  private async generateDynamicBusinessResponse(userSpecialty: string = '', userRegion: string = '') {
    ...
    console.log(`🔄 ${userSpecialty} 전문분야 동적 업체 검색 시작 - 지역: ${userRegion}`);
    
    // 1단계: 시너지 분야 동적 생성
    let synergyFields: string[] = [];
    try {
      synergyFields = await this.generateSynergyFields(userSpecialty);
      console.log(`📝 시너지 분야 생성 결과: ${synergyFields.join(', ')}`);
    } catch (fieldError) {
      console.error('시너지 분야 생성 실패:', fieldError);
      synergyFields = ['협업업체', '서비스업체', '유통업체', '마케팅업체', '제조업체'];
    }

    // 2단계: 생성된 시너지 분야를 기반으로 실제 업체 검색
    try {
      const synergyFieldsText = synergyFields.slice(0, 5).join(', ');
...
- 최소 5개 이상 추천`
    ...
    // 최종적으로 실패한 경우
    throw new Error(`'${userSpecialty}' 전문분야에 대한 실제 업체 데이터를 현재 조회할 수 없습니다. Gemini API 서비스 상태를 확인 중이니 잠시 후 다시 시도해주세요.`);
  }
  ...
  async generateSynergyFields(userSpecialty: string): Promise<string[]> {
    try {
      ...
      const response = await this.ai.models.generateContent({
        ...
        contents: [
          `"${userSpecialty}" 전문분야와 협업 및 시너지를 낼 수 있는 업종 분야를 8-10개 추천해주세요.
...
요구사항:
1. ${userSpecialty}와 실제로 협업 가능한 분야만 제시
2. 각 분야는 2-3단어로 간결하게
3. 다양한 협업 관계 포함 (공급, 유통, 마케팅, 서비스 등)
4. 한 줄에 하나씩, "-" 기호로 시작

${userSpecialty}의 특성을 고려하여 실제 비즈니스 협업이 가능한 분야들을 제시해주세요.`
        ]
      });
      ...
      if (responseText) {
        ...
        console.log(`✅ ${userSpecialty} 시너지 분야 생성 완료:`, fields);
        return fields;
      }
