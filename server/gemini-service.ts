  ...
  async searchRegionalBusinesses(searchQuery: string, userSpecialty: string = '일반', userRegion: string = '강남구'): Promise<{ businesses: any[] }> {
    try {
      ...
      const response = await this.ai.models.generateContent({
        ...
        contents: [
          `당신은 한국의 실제 비즈니스 데이터베이스에 접근할 수 있는 전문 리서치 AI입니다.

**중요**: 반드시 실제로 존재하는 업체만 추천해주세요. 가상의 업체나 임의로 만든 정보는 절대 제공하지 마세요.

검색 조건:
- 사용자 전문분야: ${userSpecialty}
- 검색 지역: 서울 ${userRegion}
- 목표: ${userSpecialty} 사업자와 실제 협업이 가능한 기존 업체들

요구사항:
1. 서울 ${userRegion} 지역에 실제로 위치한 업체만 검색
2. ${userSpecialty}와 명확한 시너지 효과를 낼 수 있는 업체
3. 실제 존재하는 업체명, 주소, 연락처 정보 제공
4. 각 업체의 실제 웹사이트 URL (있는 경우만)
5. 구체적이고 현실적인 협업 방안 제시

검색 분야 예시:
- ${userSpecialty}와 연관된 공급업체
- 유통 및 판매 채널 파트너
- 마케팅 및 홍보 협력업체
- 원료/자재 공급업체
- 가공 및 제조 파트너
  ...
  "businesses": [
    {
      ...
      "phone": "실제 연락처 (있는 경우)",
      "website": "실제 웹사이트 URL (있는 경우)",
      "synergyPotential": "${userSpecialty}와의 구체적 협업 방안",
      "description": "업체 실제 사업 내용"
    }
  ]
}

**주의**: 추측이나 임의의 정보가 아닌, 실제 확인 가능한 업체 정보만 제공해주세요.`
        ]
      });
  ...
  private async generateDynamicBusinessResponse(userSpecialty: string = '', userRegion: string = '') {
    ...
    try {
      ...
      const retryResponse = await this.ai.models.generateContent({
        ...
        contents: [
          `서울 ${userRegion} 지역에서 "${userSpecialty}" 사업자와 협업할 수 있는 실제 존재하는 업체들을 찾아주세요.
...
연락처: [실제 전화번호 또는 정보 없음]
웹사이트: [실제 URL 또는 정보 없음]
협업방안: [${userSpecialty}와의 구체적 협업 내용]
---

최소 5개 이상의 실제 업체를 추천해주세요. 가상의 정보는 절대 포함하지 마세요.`
        ]
      });
