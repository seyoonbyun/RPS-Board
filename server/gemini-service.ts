...
- 최소 5개 이상 추천`
    ...
    console.log('🔄 최종 시도: gemini-2.5-flash로 간단한 지역 업체 검색');
    try {
      ...
      const finalResponse = await this.ai.models.generateContent({
        ...
        contents: [`서울 ${userRegion}에서 "${userSpecialty}" 전문분야와 실제로 협업 가능한 업체들을 찾아주세요.

요구사항:
- 실제로 존재하는 업체만 추천
- 각 업체당 다음 정보: 업체명, 업종, 간략주소, 협업방안
- 총 3-5개 업체

예시 형식:
1. [업체명] - [업종] - [지역] - [협업방안]

딸기농장과 협업할 수 있는 실제 업체들:`]
      });
    ...
    for (const line of lines) {
      ...
      for (const pattern of patterns) {
        ...
        if (match) {
          ...
          if (name && category && name.length > 1 && category.length > 1) {
            businesses.push({
              name,
              category,
