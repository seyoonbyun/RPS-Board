...
export class GeminiService {
  ...
  async analyzeSpecialtyAndRecommendSynergies(specialty: string): Promise<{
    ...
    try {
      // Gemini API를 사용하여 실제 전문분야별 분석 수행
      const prompt = `당신은 BNI(Business Network International) 한국 지부의 비즈니스 네트워킹 전문가입니다. 
"${specialty}" 전문분야에 대한 심층적인 비즈니스 시너지 분석을 수행해주세요.

다음 구조에 맞춰 상세하고 전문적인 분석을 제공해주세요:

# ${specialty} 전문분야 심층 분석 및 비즈니스 시너지 전략

## 🏢 1. 전문분야 핵심 역량 분석
${specialty} 전문가의 핵심 업무와 전문 지식, 시장에서의 역할을 상세히 분석해주세요.

## 🤝 2. 시너지 창출 가능 비즈니스 분야
${specialty}와 협업하여 상호 시너지를 창출할 수 있는 다양한 비즈니스 분야들을 카테고리별로 분류하여 제시해주세요. 각 분야마다 구체적인 협업 방안과 시너지 효과를 설명해주세요.

## 🎯 3. 협업 우선순위 및 실행 전략
다음과 같이 시간대별로 협업 전략을 제시해주세요:
- 단기 협업 전략 (즉시~6개월)
- 중기 협업 전략 (6개월~2년)  
- 장기 협업 전략 (2년 이상)

## 💡 4. 비즈니스 시너지 극대화 방안
실제 실행 가능한 구체적인 방안들을 제시해주세요.

한국의 비즈니스 환경과 BNI 네트워킹 특성을 반영하여 실용적이고 구체적인 분석을 제공해주세요. 분석 결과는 약 800-1000자 정도로 상세하게 작성해주세요.`;
