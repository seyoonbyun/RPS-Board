...
def normalize(phone: str) -> str:
    """`010-1234-5678` · `+82 10 …` · `1012345678` → `01012345678`.

    ⛔ **구글시트가 앞자리 0 을 먹는다.** 셀에 `01062929277` 을 치면 숫자로 보고
      `1062929277` 로 저장한다 — 그걸 그대로 버리면 담당자가 번호를 넣었는데도
      "미등록" 으로 남는다(2026-08-24 실제로 그랬다). 0 이 떨어진 형태를 되살린다.
    """
    d = re.sub(r"\D", "", str(phone or ""))
    if d.startswith("8210"):
        d = "0" + d[2:]
    # 0 이 떨어진 휴대폰 번호 — 010/011/016~019 로 시작하고 9~10자리
    if re.fullmatch(r"1[016789]\d{7,8}", d):
        d = "0" + d
