...
export function PartnerRecommendations({ userId }: PartnerRecommendationsProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* AI 분석 중 로딩 팝업 */}
      <Dialog open={isLoadingAI} onOpenChange={(open) => { if (!open) setIsLoadingAI(false); }}>
      ...
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-2xl font-bold">AI의 파워팀 전략 분석</h2>
      </div>
          ...
          <Card>
            <CardHeader>
              ...
              <div className="flex flex-col md:flex-row md:items-center gap-2">
                ...
                {/* 모바일에서만 버튼 표시 */}
                {regionalBusinesses && regionalBusinesses.length > 0 && (
                  <Button size="sm" className="h-6 px-2 text-xs bg-green-600 hover:bg-green-700 text-white md:hidden w-fit self-end">
                    {regionalBusinesses.length} 업체 추천
                  </Button>
                )}
              </div>
